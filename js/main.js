$(document).ready(function(){
    ...
    //메인 비주얼 슬라이드
    // .num의 li a에 클릭이벤트 설정(01,02,03이벤트)
    ...
    // .btn a에 클릭이벤트 설정, .btn a를 클릭하면 자동슬라이드가 멈추고,다시 클릭하면 자동슬라이드 실행,
    // 자동슬라이드가 멈추고, 다시 클릭하면 자동슬라이드 실행, 버튼 모양도 바뀜
    var sw=0; //스위치변수(버튼 1개 클릭할 때마다 상태가 변할 떄 사용함)
    $('.btn a').click(function(){
        // 만약 sw변구의 값이 0이면 sw변수의 값을 1로 수정
        if(sw==0){
            sw=1;
            // 슬라이드 자동실행 멈춤
            clearInterval(auto);
   ...
   $('.product_list ul li').click(function(){
    ...
    //  자동슬라이드에 마우스오버했을 때 멈투고,마우스 아웃했을 때 다시 자동 슬라이드 됨
